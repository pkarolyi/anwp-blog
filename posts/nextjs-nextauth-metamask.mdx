---
title: "Next.js: How to authenticate with Metamask through NextAuth"
author:
  name: Peter Karolyi
  imageSrc: /authors/pkarolyi.jpg
  url: "https://github.com/pkarolyi"
date: "2023-10-19"
description: Learn how to authenticate users with their Metamask wallets using NextAuth in Next.js.
summary: "This tutorial will show you how to authenticate with Metamask using Next.js and NextAuth.js. The same structure and idea can be used for authentication using any other crypto wallet that has a browser extension..."
tags: [frontend, next.js, nextauth.js, metamask]
---
...
This tutorial will show you how to authenticate with [Metamask](https://metamask.io/) using [Next.js](https://nextjs.org) and [NextAuth.js](https://next-auth.js.org). The same structure and idea can be used for authentication using any other crypto wallet that has a browser extension.
...
The full implementation can be found on GitHub: [https://github.com/pkarolyi/nextjs-nextauth-metamask](https://github.com/pkarolyi/nextjs-nextauth-metamask).
...
To get started create a new Next.js project based on the official Next.js documentation (if asked, use the `pages` router):
...
See the completed schema in `prisma/schema.prisma`.
...
The full implementation of this endpoint with explanatory comments is in the repo at [pages/api/auth/crypto/generateNonce.ts](https://github.com/pkarolyi/nextjs-nextauth-metamask/blob/main/pages/api/auth/crypto/generateNonce.ts).

Other than that we need to configure NextAuth to accept our custom authentication method. We need to create `[...nextauth].ts` in `/pages/api/auth`. The configuration options are available in the NextAuth [documentation](https://next-auth.js.org/configuration/options).
...
The full implementation can be found in the repository under [pages/api/auth/[...nextauth].ts](https://github.com/pkarolyi/nextjs-nextauth-metamask/blob/main/pages/api/auth/%5B...nextauth%5D.ts).

### Create frontend components

First we replace the contents of `pages/index.tsx` with the following:
...
Then `pages/auth.tsx` is created as follows:
...
- As this is just a barebones example feel free to raise any security issues in the Github repo.
